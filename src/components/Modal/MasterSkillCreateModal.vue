<template>
    <SimpleModal :id="id" :isLoading="isLoading" @onSaveClick="addSkill"
        title="新增/移除技能" @modalHidden="clearModalData" :formErrorMessage="formErrorMessage">
        <template #modal-body>
            <b-form>
                <b-tags size="lg" v-model="skillList"
                    tag-variant="success" :tag-validator="tagValidator" @tag-state="onTagState">
...
<script>
    ...
    export default {
        ...
        async created(){
            this.options = [];
            this.optionTexts = [];
            const skillItems = await tigermaster.database.query('skill_item').where('skill_item.active', '!=', 0).get();
        ...
        methods:{
            async addSkill() {
                this.isLoading = true;
                try{
                    const newSkills = [...this.existSkill, ...this.skillList];
                    const skillStr = newSkills.join(',');
                    await this.currentUser.update({master:{id: this.currentUser.data.master.id, skillItems: skillStr}});
